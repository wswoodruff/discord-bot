...
const TEXT_RESPONSE_MAP = [
    ...
    { partial: ['days until christmas', 'days til christmas'], response: () => {
        ...
        // Set Christmas date for this year (December 25th)
        const christmasDate = new Date(currentYear, 11, 25); // Months are zero-based (0 = January, 11 = December)

        // If today is past Christmas, set it for next year
        if (today > christmasDate) {
            christmasDate.setFullYear(currentYear + 1);
        }

        // Calculate the difference in milliseconds
        const diffInMilliseconds = christmasDate - today;

        // Convert from milliseconds to days
        const daysUntil = Math.ceil(diffInMilliseconds / (1000 * 60 * 60 * 24)); // Convert to days
