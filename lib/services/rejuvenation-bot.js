...
module.exports = class RejuvenationBotService extends Schmervice.Service {
    ...
    async onMessageEdit(message) {
        ...
        try {
            ...
            const oldNewMsg = await message.fetch();
            console.log('oldNewMsg.content', oldNewMsg.content);
            console.log('oldNewMsg', oldNewMsg);

            await new Promise((res) => setTimeout(res, 5000));

            const newMsg = await message.fetch();

            console.log('message.content', newMsg.content);
            ...
            const richMessage = discordService.buildMessage({
                ...
                fields: [
                    ...
                    { name: 'Edited Message', value: `${message.content || '*No content*'}`, inline: true }
                ],
