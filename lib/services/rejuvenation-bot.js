    ...
    async onMessageEdit(oldMessage, newMessage) {
        ...
        try {
            ...
            for (const [ownerId, owner] of owners) {
                try {
                    await owner.send(richMessage);
                    console.log(`Notified moderator ${owner.user.tag} (ID: ${ownerId}) about the edited message.`);
                }
                catch (error) {
                    console.error(`Failed to send DM to moderator ${owner.user.tag} (ID: ${ownerId}):`, error);
                }
            }
        }
        catch (error) {
